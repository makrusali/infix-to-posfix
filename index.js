...
window.onload = function () {
    ...
    const modalBtnEl = document.getElementById('btn-modal');
    ...
    function infix2Posfix(infix) {
        ...
        for (let i = 0; i < splitedLength; i++) {
            ...
            if (isOperand(token)) {
                ...
                if (isInvalid == false) {
                    ...
                    modalMessageEl.innerHTML += arrInfixSplited.filter((v) => {
                        if (v != ';') return v;
                    }).join(' ') + '\n';
                }
                ...
                //     messageModalEl
                // }
                // modalMessageEl.innerHTML += `[invalid token : ${token} | pos: ${i}]\n`;
                isInvalid = true;
            }
        ...
        if (isInvalid == true) {
            ...
            let invalidPointer = '';
            let invalidPosI = 0;
            for (let i = 0; i < arrInfixSplited.length; i++) {
                if (i == invalidPos[invalidPosI]){
                    invalidPointer += '^ ';
                    invalidPosI++;
                } else {
                    invalidPointer += '  ';
            ...
            modalMessageEl.innerHTML += invalidPointer; 
            modalMessageEl.innerHTML += '\n\n' + invalidToken.map((v)=>{
                return '❌ ' + v;
            }).join('\n');
            return;
    ...
    modalBtnEl.addEventListener('click', (e) => {
        modalEl.classList.add('modal__hidden');
    ...
    btnConvertEl.addEventListener('click', (e) => {
        ...
        // stop, if result is undefined
        if (posfixToken === undefined) return;
